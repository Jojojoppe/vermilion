...
void Vermilion::Core::Vulkan::Texture::setData(void * data){
    stbi_uc * pixels = (stbi_uc*) data;
    ...
    VkResult res = vmaCreateBuffer(this->api->vma_allocator, &bufferInfo, &allocInfo, &stagingBuffer, &stagingBufferMemory, nullptr);
    if( res != VK_SUCCESS){
        this->instance->logger.log(VMCORE_LOGLEVEL_FATAL, "Could not create image staging buffer: %d", res);
