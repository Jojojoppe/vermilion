...
void Vermilion::Core::Instance::getDefaultRenderTarget(VmRenderTarget& renderTarget){
	auto defrentar = this->api->getDefaultRenderTarget();
	renderTarget.ID = 0;
	renderTarget.instance = this;
}

void Vermilion::Core::Instance::createRenderTarget(VmRenderTarget& renderTarget, VmTexture& texture){
	if(texture.ID==0){
	...
	renderTarget.ID = IDcounter++;
	renderTarget.instance = this;
}

void Vermilion::Core::Instance::createShader(VmShader& s, const std::string& source, Vermilion::Core::ShaderType type){
...
void Vermilion::Core::Instance::createShaderProgram(VmShaderProgram& s, std::initializer_list<VmShader*> shaders){
...
void Vermilion::Core::Instance::createPipeline(VmPipeline& p, VmRenderTarget& renderTarget, VmShaderProgram& shaderProgram, VmPipelineLayout& pipelineLayout, Vermilion::Core::PipelineSettings settings){
...
void Vermilion::Core::Instance::createBinding(VmBinding& b, std::initializer_list<VmBuffer*> buffers, std::initializer_list<VmSampler*> samplers){
...
void Vermilion::Core::Instance::createRenderable(VmRenderable& r, VmBuffer& vertexBuffer, VmBuffer& indexBuffer, unsigned int vertexOffset, unsigned int indexOffset, unsigned int length){
...
void Vermilion::Core::Instance::createTexture(VmTexture& t, size_t width, size_t height, unsigned int channels){
...
void Vermilion::Core::Instance::createSampler(VmSampler& s, VmTexture& texture){
	auto tex = this->textures[texture.ID];
